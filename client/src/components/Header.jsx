...
const Header = () => {
    ...
    return (
        <motion.div
            className='flex flex-col justify-center items-center text-center mt-20 mb-10'
        ...
        >
            ...
            >
                {Array(6).fill('').map((item, index) => (
                    <motion.img
                        className='rounded hover:scale-105 transition-all duration-300 cursor-pointer max-sm:w-25'
                        width={200}
                        key={index}
            ...
            <motion.p
                className='mt-4 text-neutral-600 text-2xl max-sm:text-xl'
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ delay: 1.2, duration: 0.8 }}
            >
                Generated images from imaginary
            </motion.p>
        </motion.div>
    )
}
